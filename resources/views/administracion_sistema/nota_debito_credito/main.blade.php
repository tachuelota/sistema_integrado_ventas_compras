                        ...
                        <table class="table table-striped text-center"  datatable="ng" dt-instance="dtInstance" style="font-size: 12px;">
                            ...
                            <tbody >
                                <tr ng-repeat="x in NotaCtrl.notas">
                                    <td>/% $index + 1 %/ </td>
                                    <td>/% "000".substring(0, 3 - x.nota.serie_nota.length) + x.nota.serie_nota %/</td>
                                    <td>/% "00000000".substring(0, 8 - x.nota.numero_nota.length) + x.nota.numero_nota %/ </td>
                                    <td>/% x.nota.tipo_nota.nombre_tipo_nota %/ </td>
                                    <td>/% x.nota.moneda.nombre_moneda %/ </td>
                                    <td>/% x.nota.fecha %/ </td>
                                    <td ng-if="x.nota.moneda.id == 1">/% NotaCtrl.formatNumber(x.nota.monto_total, "S/. ") %/ </td>
                                    <td ng-if="x.nota.moneda.id == 2">/% NotaCtrl.formatNumber(x.nota.monto_total, "$ ") %/ </td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="" data-toggle="tooltip" title="Eliminar Factura" ng-click="NotaCtrl.deleteAttempt(x.nota.id)"><i class="fa fa-trash fa-2x"></i></a> 
                                    </td> -->
                                </tr>
                            </tbody>
                        </table>
...
<script type="text/javascript">
    ...
    app.factory('DetalleNotaFactory', function DetalleNotaFactory($http, $modal, $filter ) {
        ...
        angular.element(div_loader).removeClass('hide');
            return $http.get('AdsDetalleNota/getAllComprobantes').success(function(data) {
                    ...
                    $scope.asociar = function(){
                        ...
                        $http.post('AdsComprobanteVenta/getFacturaBySerieNumero', {guia:$scope.factura})
                            .success(function(data){
                                ...
                                if(data.datos == ''){
                                    sweetAlert("Error", "Esta Factura de Venta no existe", "error");
                                }
                                else{

                                    $http.post('AdsDetalleNota/asociar_factura', {nota:scope, comprobante:data.datos[0], tipo_comprobante: 'venta'})
